...
public class WebLogAspect {
    /**
     * ThreadLocal可以让我们拥有当前线程的变量，可以通过set（）和get（）来对这个局部的变量进行操作，
     * 不会和其他的线程的局部变量进行冲突，实现了线程的数据隔离
     * 此处用他进行日志信息在各个方面之间的共享
     */
    private ThreadLocal<Map<String,Object>> threadLocal = new ThreadLocal<>();

    /**
     * 定义切点，横切controller包下的所有共有方法
     */
    ...
    public void doBefore(JoinPoint joinPoint, ControllerWebLog controllerWebLog) {
        ...
        log.info("连接点对象通过反射获得类和方法名：" + joinPoint.getSignature().getDeclaringTypeName() + "," + joinPoint.getSignature().getName());
        log.info("AOP拦截获得参数：" + Arrays.toString(joinPoint.getArgs()));
        log.info("执行方法：" + controllerWebLog.name());
        // 定义一个map用来记录日志的信息，并将其put到threadLocal
    ...
    public void doAfterReturning(ControllerWebLog controllerWebLog,Object ret) throws Throwable{
        ...
        // 将请求到的目标方法getHello()的执行的返回结果存入线程对象
        threadInfo.put("result",ret);
        if (controllerWebLog.isSaved()){
            // 真实场景可做定时插入数据操作，此处仅模拟
            log.info("日志已存入数据库");
        }
        // 处理完毕请求，返回内容
